...
exports.canvas = function() {
    ...
    canvas.blueberry = wrap(blueberry);
...
// src/filters/fun/blueberry.js
/**
 * @filter           Hope Poster
 * @description      Posterize the image using the colors in the famous Obama "hope" poster.
 */
function blueberry(centerX, centerY, angle, radius, isInfinite) {
    gl.blueberry = gl.blueberry || new Shader(null, '\
        uniform sampler2D texture;\
        uniform vec2 texSize;\
        varying vec2 texCoord;\
        \
    ...
    simpleShader.call(this, gl.blueberry, {
        texSize: [this.width, this.height]
    });
 ...
 * @description      Posterize the image using the colors in the famous Obama "hope" poster.
 */
function flatColors(centerX, centerY, angle, radius, isInfinite) {
    gl.flatColors = gl.flatColors || new Shader(null, '\
        uniform sampler2D texture;\
        uniform vec2 texSize;\
        varying vec2 texCoord;\
        \
        void main() {\
            vec4 c = texture2D(texture, texCoord).rgba;\
            c.r = (floor((c.r * 255.0) / 32.0) * 32.0)/255.0;\
            c.g = (floor((c.g * 255.0) / 24.0) * 24.0)/255.0;\
            c.b = (floor((c.b * 255.0) / 32.0) * 32.0)/255.0;\
            gl_FragColor = c;\
        }\
    ');

    simpleShader.call(this, gl.flatColors, {
        texSize: [this.width, this.height]
    });
...
// src/filters/video/popArt.js
/**
 * @filter      Andy
 * @description Makes the image look like Andy Warhol's soup.
 */
function popArt() {
    gl.popArt = gl.popArt || new Shader(null, '\
        uniform sampler2D texture;\
        uniform vec2 texSize;\
        varying vec2 texCoord;\
        \
    ...
    simpleShader.call(this, gl.popArt, {
        texSize: [this.width, this.height]
    });
